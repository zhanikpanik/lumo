...
export const CloseShiftScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const handleConfirmClose = async () => {
    ...
    setClosing(true);
    const closed = await closeShift(n);
    if (!closed) {
      ...
      <View style={styles.root}>
        ...
        <View style={styles.body}>
          ...
          <View style={styles.leftCol}>
            ...
            <StatRow label="Кассир" value={currentShift.cashier} />
            <StatRow label="Открыта" value={formatTime(currentShift.openedAt)} />
            <StatRow label="Длительность" value={duration} />

            <View style={styles.divider} />

            <StatRow label="Заказов" value={String(currentShift.totalOrders)} bold />
            <AmountRow label="Выручка" amount={currentShift.totalRevenue} bold />

            <View style={styles.divider} />
            <Text style={styles.sectionTitle}>По оплатам</Text>

            <AmountRow label="Наличные" amount={currentShift.cashTotal} sub={`${currentShift.cashPayments} зак.`} />
            <AmountRow label="Карта" amount={currentShift.cardTotal} sub={`${currentShift.cardPayments} зак.`} />
            <AmountRow label="Без оплаты" amount={currentShift.otherTotal} sub={`${currentShift.otherPayments} зак.`} />

            <View style={styles.divider} />
            <Text style={styles.sectionTitle}>Движения</Text>

            <AmountRow label="Внесения" amount={currentShift.cashFloatIn ?? 0} accent />
            <AmountRow label="Изъятия" amount={currentShift.cashFloatOut ?? 0} />
            <AmountRow label="Инкассация" amount={currentShift.cashCollectionsTotal ?? 0} />
