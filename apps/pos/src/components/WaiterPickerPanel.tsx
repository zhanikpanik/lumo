...
interface WaiterPickerPanelProps {
  onChangeWaiter?: (employeeId: string) => void;
}

export const WaiterPickerPanel: React.FC<WaiterPickerPanelProps> = ({ onChangeWaiter }) => {
  const { employees } = useInstantVenue();
  const currentWaiter = ''; // Resolved by caller via InstantDB
  const waiters = employees.map(e => ({ name: e.name, id: e.id }));
  type Cell = 
              ...
              return (
                <View key={key} style={[styles.cellWrap, ci < COLS - 1 && { marginRight: GAP }]}>
                  <TouchableOpacity
                    ...
                    onPress={() => onChangeWaiter?.(cell.id)}
                    activeOpacity={0.7}
                  >
                    <Text
