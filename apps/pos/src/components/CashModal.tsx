...
export const CashModal: React.FC<Props> = ({
  ...
  const { openShift: currentShift, expectedCashTiyin, cashTotalTiyin, cashFloatInTiyin, cashFloatOutTiyin, cashCollectionsTiyin } = useInstantShift(currentUser?.id);
  ...
  const expectedCash = expectedCashTiyin ?? (startingCashTiyin ?? 0) + (cashTotalTiyin ?? 0);
          ...
          <View style={styles.body}>
            {!currentShift ? (
              ...
              <>
                ...
                <InfoRow label="Наличные на начало" amount={startingCashTiyin ?? 0} />
                <InfoRow label="Поступления наличными" amount={cashTotalTiyin ?? 0} />

                <View style={styles.divider} />

                <InfoRow label="Внесения" amount={cashFloatInTiyin ?? 0} accent />
                <InfoRow label="Изъятия" amount={cashFloatOutTiyin ?? 0} />
                <InfoRow label="Инкассация" amount={cashCollectionsTiyin ?? 0} />
