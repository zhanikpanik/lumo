...
interface Props {
  ...
  hideRight?: boolean;
  /** InstantDB: order snapshot for display (when provided, skips useOrderStore lookup) */
  currentOrder?: { number?: string; tableNumber?: string; guestCount?: number; zone?: string; waiter?: string } | null;
}
...
export const PosHeader: React.FC<Props> = ({
  ...
  useEffect(() => {
    intervalRef.current = setInterval(() => {
      setTimeStr(new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }));
    }, 60_000);
