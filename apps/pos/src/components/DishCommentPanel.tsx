...
interface DishCommentPanelProps {
  /** Order items from parent (InstantDB or draft) */
  items: OrderItem[];
  /** Called to set comment on a committed (non-draft) item. If omitted, uses setDraftComment from posUiStore. */
  onSetComment?: (itemId: string, comment: string) => void;
}

export const DishCommentPanel: React.FC<DishCommentPanelProps> = ({ items, onSetComment }) => {
  const { selectedItemId, setActiveAction } = usePosUiStore();
  const setDraftComment = usePosUiStore((s) => s.setDraftComment);
  ...
  const setComment = (comment: string) => {
    if (onSetComment) {
      onSetComment(selectedItem.id, comment);
    } else {
