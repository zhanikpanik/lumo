...
export const CashScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const { openShift, totalOrders, expectedCashTiyin, cashTotalTiyin, cardTotalTiyin, otherTotalTiyin, cashFloatInTiyin, cashFloatOutTiyin, cashCollectionsTiyin } = useInstantShift(currentUser?.id);
  ...
  const expectedCash = expectedCashTiyin ?? (openShift?.startingCashTiyin ?? 0) + (cashTotalTiyin ?? 0);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!Number.isSafeInteger(amountTiyin)) {
  ...
  const renderLeftStats = () => (
    <View style={styles.statsPanel}>
      ...
      <StatRow label="В кассе на начало" amount={openShift?.startingCashTiyin ?? 0} />
      <StatRow label="Оплаты наличными" amount={cashTotalTiyin ?? 0} />
      <StatRow label="Заказов всего" value={String(totalOrders ?? 0)} />
      <View style={styles.divider} />
      <Text style={styles.sectionTitle}>По оплатам</Text>
      <StatRow label="Наличные" amount={cashTotalTiyin ?? 0} />
      <StatRow label="Карта" amount={cardTotalTiyin ?? 0} />
      <StatRow label="Без оплаты" amount={otherTotalTiyin ?? 0} />
      <View style={styles.divider} />
      <Text style={styles.sectionTitle}>Движения</Text>
      <StatRow label="Внесения" amount={cashFloatInTiyin ?? 0} accent />
      <StatRow label="Изъятия" amount={cashFloatOutTiyin ?? 0} />
      <StatRow label="Инкассация" amount={cashCollectionsTiyin ?? 0} />
    </View>
  );
