...
export function ShiftsList({ data, isPending, error }: Props) {
  if (isPending) {
    return (
  ...
  if (error || !data) {
    return (
      <div className="bg-card rounded-lg border border-border/60 p-4">
        <h3 className="text-base font-semibold text-foreground mb-3">Смены</h3>
  ...
  if (shiftStats.length === 0) {
    return (
      <div className="bg-card rounded-lg border border-border/60 p-4">
        <h3 className="text-base font-semibold text-foreground mb-3">Смены</h3>
        <div className="py-8 text-center text-sm text-muted-foreground">
          Нет данных о сменах за период
        </div>
      </div>
    );
  }

  const maxRevenue = shiftStats[0]?.revenue || 1;

  return (
    <div className="bg-card rounded-lg border border-border/60 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-base font-semibold text-foreground">Смены</h3>
      ...
      <div className="space-y-1">
        {shiftStats.slice(0, 10).map((s) => (
          ...
          >
              ...
              <div className="flex-1 h-7 bg-muted rounded-r-sm overflow-hidden relative">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: s.marginPct >= 50
                      ? CHART_GREEN_SOLID
                      : s.marginPct >= 30
                        ? '#eab308'
                        : CHART_RED_SOLID,
                    opacity: 0.55 + (s.revenue / maxRevenue) * 0.45,
                  }}
                />
              </div>
              <span className="text-sm font-medium tabular-nums text-foreground w-16 text-right shrink-0">
                {fmtSom(s.revenue)} сом
              </span>
            </div>
            {/* Margin pct */}
            <span
            ...
            {/* Cashier name */}
            <span className="text-sm text-foreground truncate w-28 shrink-0 text-right">
              {s.cashierName}
            </span>
          </div>
        ))}
      ...
      <p className="text-[10px] text-muted-foreground mt-2 leading-tight">
        Привязка к смене — по кассиру. Если в смене работали несколько человек, attribution требует performed_by.
      </p>
    </div>
  );
}
